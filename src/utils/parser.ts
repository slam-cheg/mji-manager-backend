  ...
  while (rowIndex < data.length) {
    ...
    if (colA && section.hasOwnProperty(colA)) {
      ...
      if (MULTILINE_KEYS.includes(currentKey)) {
        ...
        typeof section[currentKey] === "object" &&
        !Array.isArray(section[currentKey])
      ) {
...
function parseFlatElement(data: string[][], row: number, target: any): number {
  ...
  while (row < data.length) {
    const [colA, colB, colC, colD, colE, colF, colG] = data[row].map(
      (v) => v?.trim() ?? "",
    );
    ...
    if ("% деф. части" in target)
      target["% деф. части"] = parseFloat(colF) || 0;
...
function parseMultilineElement(
  data: string[][],
  row: number,
  section: any,
): number {
