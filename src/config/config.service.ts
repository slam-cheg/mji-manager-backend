...
/** Промпт для перефразирования описания дефекта. Подстановка: {{text}} — текст дефекта. */
export const DEFAULT_REPHRASE_DEFECT_PROMPT = `Перефразируй следующее описание дефекта в отчёте по обследованию здания. Требования: только инженерный язык, не длиннее двух предложений. Верни только перефразированный текст, без кавычек и пояснений.
...
export class ConfigService {
  ...
  /** Настройки AI (модель и промпты). Системный промпт не редактируется — всегда из кода. */
  getAiSettings(): AiSettings {
    ...
    const defaults: AiSettings = {
      ...
      rephraseDefectPrompt: DEFAULT_REPHRASE_DEFECT_PROMPT,
    };
    try {
      ...
      return {
        ...
        rephraseDefectPrompt:
          typeof parsed.rephraseDefectPrompt === "string"
            ? parsed.rephraseDefectPrompt
            : defaults.rephraseDefectPrompt,
      };
    } catch (error) {
